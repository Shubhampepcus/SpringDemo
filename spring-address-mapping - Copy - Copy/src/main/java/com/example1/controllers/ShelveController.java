...
@RestController @RequestMapping("/shelves") public class ShelveController {
  @Autowired ShelveService shelveService;

  @PostMapping("/create-shelves") public ResponseEntity <Shelve> addShelve(@RequestBody Shelve shelve) {
    return new ResponseEntity(shelveService.saveShelve(shelve), HttpStatus.CREATED);
  }

  @PatchMapping("/{id}") public ResponseEntity <Shelve> addBook(@PathVariable Integer id,
